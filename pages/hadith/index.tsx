...
import SearchResultPopupHadith from "../../components/hadith/SearchResultPopupHadith";

const Hadith = () => {
  const [searchText, setSearchText] = useState("");
  const [selectedTxt, setSelectedTxt] = useState("1.txt");
  const [disabledValue, setDisabledValue] = useState(19);
  const [hadithType, setHadithType] = useState("Hadith Sahih Bukhari");
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-white">
      <h1 className="text-2xl font-bold text-gray-800 mb-6 text-center">CARIAN AYAT HADITH</h1>
      ...
      <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 mb-6">
        ...
        <select
          value={hadithType}
          onChange={(e) => setHadithType(e.target.value)}
          className="flex-1 p-3 border rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
        >
          <option>Hadith Sahih Bukhari</option>
          <option>Hadith Sahih Muslim</option>
        </select>
      </div>
